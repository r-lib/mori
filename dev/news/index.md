...
- [`share()`](https://mori.r-lib.org/dev/reference/share.md) of
...
- Region name counters now start at a per-process random value, so a
  ...
  orphaned regions ([\#50](https://github.com/r-lib/mori/issues/50)).
...
- New
  ...
  ([\#25](https://github.com/r-lib/mori/issues/25)).
- When [`share()`](https://mori.r-lib.org/dev/reference/share.md) fails
  to create a shared memory region it now reports the requested size
  and, where applicable, an actionable hint (e.g. raising a container’s
  `/dev/shm` limit) instead of a generic message
  ([\#29](https://github.com/r-lib/mori/issues/29)).
...
- [`share()`](https://mori.r-lib.org/dev/reference/share.md) on Linux
  ...
  ([\#14](https://github.com/r-lib/mori/issues/14)).
- [`map_shared()`](https://mori.r-lib.org/dev/reference/map_shared.md)
