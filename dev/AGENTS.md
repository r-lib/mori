...
- **Layout oracle + writer**: `mori_layout_size(x)` → region size, or 0
  for what the writer must not take (ALTREP nodes that are neither mori
  views nor directly readable with no keeper chain —
  ...
  path passes `NULL` and vetoes nothing. `mori_layout_write(base, x)`
  emits exactly `mori_layout_size` bytes and zeroes reserved header
  bytes \[24-63\] on every write (embedders may recycle regions).
